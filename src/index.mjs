...
passport.serializeUser((user, done) => {
    const key = user.googleId ? `google:${user.id}` : `local:${user.id}`;
    done(null, key);
});
...
passport.deserializeUser(async (key, done) => {
    try {
        ...
        const findUser = type === "google" ? await googleUser.findById(id) : await User.findById(id);
...
app.get("/", (request, response) => {
    ...
    response.cookie("TestCookies", "base url cookie", { maxAge: 60000 * 60 }); // expires in 1 min (unit in milliseconds)
...
app.put("/api/users/:username", checkSchema(createUserValidationSchema), async (request, response) => {
    ...
    try {
        const updatedUser = await User.findOneAndReplace({ username }, data, { new: true });
...
app.patch("/api/users/:username", async (request, response) => {
    ...
    const updatedUser = await User.findOneAndUpdate({ username }, { $set: request.body }, { new: true });
...
app.get("/api/google/auth/redirect", passport.authenticate("google"), (request, response) => {
    return response.status(200).send({ msg: "Logged in successfully" });
});
