...
describe("POST /api/users", () => {
    ...
    it("creates a second user successfully", async () => {
        const response = await request(app).post("/api/users").send(SECOND_USER);
        expect(response.status).toBe(201);
        expect(response.body).toMatchObject({ username: SECOND_USER.username });
    });

    it("returns 400 when username is missing", async () => {
        const response = await request(app).post("/api/users").send({ password: "somepass" });
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty("error");
    });

    it("returns 400 when password is missing", async () => {
        const response = await request(app).post("/api/users").send({ username: "nobody" });
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty("error");
    });
...
describe("GET /api/users", () => {
    ...
    it("returns all users when a valid filter is given but no value", async () => {
        const response = await request(app).get("/api/users").query({ filter: "username" });
        expect(response.status).toBe(200);
    ...
    it("filters users by username using a partial value", async () => {
        const response = await request(app).get("/api/users").query({ filter: "username", value: "staging_ali" });
        expect(response.status).toBe(200);
    ...
    it("returns an empty array when filter value matches nothing", async () => {
        const response = await request(app).get("/api/users").query({ filter: "username", value: "zzz_no_match_zzz" });
        expect(response.status).toBe(200);
        expect(response.body).toEqual([]);
    });

    it("filter is case-insensitive", async () => {
        const response = await request(app).get("/api/users").query({ filter: "username", value: "STAGING_ALICE" });
        expect(response.status).toBe(200);
        expect(response.body.length).toBeGreaterThanOrEqual(1);
    });
});
...
describe("GET /api/users/:username", () => {
    it("returns the user document when found", async () => {
        const response = await request(app).get(`/api/users/${TEST_USER.username}`);
        expect(response.status).toBe(200);
        expect(response.body).toMatchObject({ username: TEST_USER.username });
    });
...
describe("PUT /api/users/:username", () => {
    ...
    it("replaces the user document and returns 200", async () => {
        const response = await request(app).put(`/api/users/${TEST_USER.username}`).send(updatedPayload);
        expect(response.status).toBe(200);
        expect(response.body).toMatchObject({ username: TEST_USER.username });
    ...
    it("returns 404 when the target user does not exist", async () => {
        const response = await request(app).put("/api/users/ghost_user_xyz").send(updatedPayload);
...
describe("PATCH /api/users/:username", () => {
    ...
    it("returns 404 when the target user does not exist", async () => {
        const response = await request(app).patch("/api/users/ghost_user_xyz").send({ password: "whatever" });
        expect(response.status).toBe(404);
        expect(response.body).toEqual({ message: "User not found" });
    });
});
...
describe("DELETE /api/users/:username", () => {
    it("deletes an existing user and returns 200", async () => {
        const response = await request(app).delete(`/api/users/${SECOND_USER.username}`);
        expect(response.status).toBe(200);
...
describe("Data integrity", () => {
    it("passwords are never stored or returned in plain text", async () => {
        const response = await request(app).get("/api/users").query({ filter: "username", value: "staging" });
        expect(response.status).toBe(200);
    ...
    it("usernames are unique — duplicate registration returns 400", async () => {
        ...
        // Cleanup
        await mongoose.connection.collection("users").deleteOne({ username: "unique_check_user" });
    });
